...
class _CallScreenState extends State<CallScreen>
    ...
    return BlocConsumer<CallBloc, CallState>(
      ...
      builder: (context, state) {
        if (state is CallIncomingState) {
          return _buildIncomingRingingView(context, state);
        }
  ...
  // ─── 2. Incoming Ringing View (In-app incoming call popup view) ────────────
  Widget _buildIncomingRingingView(
    BuildContext context,
    CallIncomingState state,
  ) {
    final isVideo = state.callType == CallType.video;
                ...
                builder: (context, child) {
                  return SizedBox(
                    ...
                    child: Stack(
                      ...
                      children: [
                        Container(
                          width: 140 + (_rippleController.value * 30),
                          height: 140 + (_rippleController.value * 30),
                          decoration: BoxDecoration(
                            shape: BoxShape.circle,
                            color: const Color(0xFF8059CA).withValues(
                              alpha: 0.25 * (1.0 - _rippleController.value),
                      ...
                      GestureDetector(
                        onTap: () {
                          context.read<CallBloc>().add(
                            RejectCallEvent(callId: state.callId),
                          );
                        },
                        child: Container(
                          width: 68,
                  ...
                  Column(
                    children: [
                      GestureDetector(
                        onTap: () {
                          context.read<CallBloc>().add(
                            AcceptCallEvent(callId: state.callId),
                          );
                        },
                        child: Container(
                          width: 68,
  ...
  // ─── 3. Active Connected Call Session View (Live Audio / Video P2P) ─────────
